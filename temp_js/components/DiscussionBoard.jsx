...
export default function DiscussionBoard({ discussions, onSendMessage, currentUser }) {
    ...
    return (<div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        {discussions.map((d, index) => (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} key={d.id} className="bg-zinc-50 rounded-2xl p-5 border border-zinc-100 group hover:border-zinc-200 transition-all">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <AnimatePresence>
                  {replyingTo === d.id && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-4 overflow-hidden pl-4 border-l-2 border-indigo-100">
                      <div className="flex gap-3">
                         ...
                         <button onClick={() => { setReplyingTo(null); setReplyMessage(''); }} className="bg-indigo-600 text-white px-4 rounded-lg text-xs font-bold hover:bg-indigo-700">
                           Post
                         </button>
                      </div>
                    </motion.div>)}
